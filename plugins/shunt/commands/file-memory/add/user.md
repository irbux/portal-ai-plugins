---
description: Add one entry to the Shunt USER memory store
argument-hint: "<entry text>"
---

Use the shunt file-memory skill. Save the fact in $ARGUMENTS to the user store
with `memory add --root "$PWD" --target user`, passing the JSON payload on stdin
