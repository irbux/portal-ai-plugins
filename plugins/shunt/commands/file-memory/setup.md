...
argument-hint: "[--location plugin-data|project] [--memory-chars N] [--user-chars N]"
---

Use the shunt file-memory skill. Run `memory setup` with `--root "$PWD"` and any
arguments the user gave in $ARGUMENTS, then report the resolved store, project id and
limits. Existing settings and saved entries are preserved. Explain that memory is
scoped to this project, that USER.md does not propagate to other projects, that the
...
afterwards to confirm. Do not enable memory in a project the user did not ask about.
